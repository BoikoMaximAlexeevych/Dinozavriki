...
void make_Buffer(sf::RenderWindow& window, Scene& cs, sf::Font& font, Player& player)
{
	...
	sf::Text DISPLAY;
	{
		...
		DISPLAY.setPosition(530.f, 250.f);
	...
	// Ã≈Õﬁ
	sf::RectangleShape option_menu;
	sf::Text MENU;
	{
		option_menu.setPosition(sf::Vector2f(500.f, 550.f));
		option_menu.setSize(sf::Vector2f(280.f, 80.f));
		option_menu.setFillColor(sf::Color(162, 239, 235));
		MENU.setPosition(560.f, 550.f);
		MENU.setFont(font);
		MENU.setString("MENU");
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(ev))
		{
			...
			//GOTO MENU
			if (option_menu.getGlobalBounds().contains(sf::Mouse::getPosition(window).x, sf::Mouse::getPosition(window).y))
			{
				option_menu.setFillColor(sf::Color(199, 253, 250));
...
void make_Shop(sf::RenderWindow& window, Scene& cs, sf::Font& font, Player& player)
{
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(ev))
		{
			...
			//BUY DINO
			if (buy_button.getGlobalBounds().contains(sf::Mouse::getPosition(window).x, sf::Mouse::getPosition(window).y))
			{
				buy_button.setFillColor(sf::Color(199, 253, 250));
...
void make_Choosing(sf::RenderWindow& window, Scene& cs, sf::Font& font, Player& player)
{
	...
	sf::Text LETSGO;
	{
		...
		right_arrow.setFont(font);
		right_arrow.setString("LET'S GO!");
		right_arrow.setFillColor(sf::Color::Black);
		right_arrow.setCharacterSize(70);
		right_arrow.setPosition(sf::Vector2f(630.f, 350.f));
	}
